...
Search is cosine similarity (`1 - (embedding <=> query)`) with optional repo/branch/since/tier/exit-code filters. Postgres NOTICEs are suppressed via `onnotice`. All data paths use postgres.js tagged-template parameters; `sql.unsafe` appears only for constant DDL.
