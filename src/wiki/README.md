...
4. **Write pages** — merge frontmatter monotonically (`sources`/`trajectories` only grow), recompute page fingerprint, bump `updated`. **Anti-clobber**: `violatesShrinkGuard(old,new)` rejects a rewrite < 40% of an old body > 500 chars; ingest never deletes pages; the wiki dir is auto-`git init`-ed and every run commits (one `git revert` away).
