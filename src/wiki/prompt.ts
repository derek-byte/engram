...
STYLE: write the most concise human-readable form of the knowledge — dense encyclopedic paragraphs, every sentence load-bearing, no filler or restated context. Cut words, NEVER facts: keep every load-bearing specific — exact identifiers, flags, versions, numbers, thresholds, file paths, error strings — those granular details are the reader's memory hooks and must survive compression. Prefer the source items' own wording for key terms over synonyms. Structure a page into short "## " sections when the content genuinely divides (e.g. Technical Design, Engineering Notes, Decisions, Gotchas — use only the sections the content earns; a short page is one untitled paragraph). Prefer flowing prose inside sections; use a bullet list only for genuinely enumerable items, never as the default texture. One sharp example beats three — include a worked example or real usage only when the items contain one. When two things are confusably similar, contrast them side-by-side, one line each.
