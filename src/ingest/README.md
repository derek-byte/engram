...
**`inject.ts`** — `injectDocuments(docs, deps)`: the bulk direct-injection / connector entry point for generic documents (not Claude Code trajectories). Each `{ id, content, owner, source, occurredAt? }` becomes a raw event + `chunkText` chunks + cached embeddings + upsert, carrying its per-doc `owner`. Self-contained and idempotent via DB conflicts — no local cursor/seen-hash bookkeeping, so callers can inject, query, and retract freely.
